...
def _brand_from_emisor(
    ...
    Primary key: issuer RUC (more reliable than the corporate name that may vary).
    Fallback: corporate name (razÃ³n social).
    """

    # --- Primary: RUC based routing (REINE SRL group: Estilo Home / Mayoristas Uruguay) ---
    try:
        r = "".join(re.findall(r"\d+", str(ruc or "")))
    except Exception:
        r = ""
    if r == "218959840015":
        # Desempate por sucursal (CdgDGISucur):
        #   - 1 => Estilo Home
        #   - 2 => Mayoristas Uruguay
        if int(sucursal or 0) == 2:
            return "MAYORISTAS_URUGUAY"
        return "ESTILO_HOME"

    # --- Fallback: corporate name based routing ---
    rs = (razon_social or "").strip().upper()
    # Normalize punctuation/spaces for robust matching
    rs = re.sub(r"[^A-Z0-9]+", " ", rs)
    rs = re.sub(r"\s+", " ", rs).strip()
    # REINE S.R.L. / ESTILO HOME => Estilo HOME
    if re.search(r"\bREINE\b", rs) or re.search(r"\bESTILO\b\s+\bHOME\b", rs) or re.search(r"\bESTILOHOME\b", rs):
        return "ESTILO_HOME"

    # Mayoristas Uruguay (por razón social)
    if re.search(r"\bMAYORISTAS\b", rs) and re.search(r"\bURUGUAY\b", rs):
        return "MAYORISTAS_URUGUAY"
...
UY_LATAM_DOCUMENT_TYPE_NAME_BY_CODE: dict[int, str] = {
    101: "e-Ticket",
    102: "Nota de CrÃ©dito de e-Ticket",
    103: "Nota de DÃ©bito de e-Ticket",
    111: "e-Factura",
    112: "Nota de CrÃ©dito de e-Factura",
    113: "Nota de DÃ©bito de e-Factura",
    121: "e-Factura ExportaciÃ³n",
    122: "Nota de CrÃ©dito de e-Factura ExportaciÃ³n",
    123: "Nota de DÃ©bito de e-Factura ExportaciÃ³n",
    124: "e-Remito ExportaciÃ³n",
    131: "e-Ticket Venta por Cuenta Ajena",
    132: "Nota de CrÃ©dito e-Ticket Venta por Cuenta Ajena",
    133: "Nota de DÃ©bito e-Ticket Venta por Cuenta Ajena",
    141: "e-Factura Venta por Cuenta Ajena",
    142: "Nota de CrÃ©dito e-Factura Venta por Cuenta Ajena",
    143: "Nota de DÃ©bito e-Factura Venta por Cuenta Ajena",
    151: "e-Boleta",
    152: "Nota de CrÃ©dito e-Boleta",
    153: "Nota de DÃ©bito e-Boleta",
    181: "e-Remito",
    182: "e-Resguardo",
    ...
    212: "Nota de CrÃ©dito de e-Factura Contingencia",
    213: "Nota de DÃ©bito de e-Factura Contingencia",
    221: "e-Factura ExportaciÃ³n Contingencia",
    222: "Nota de CrÃ©dito de e-Factura ExportaciÃ³n Contingencia",
    223: "Nota de DÃ©bito de e-Factura ExportaciÃ³n Contingencia",
    224: "e-Remito de ExportaciÃ³n Contingencia",
    231: "e-Ticket Venta por Cuenta Ajena Contingencia",
    232: "Nota de CrÃ©dito de e-Ticket Venta por Cuenta Ajena Contingencia",
    233: "Nota de DÃ©bito de e-Ticket Venta por Cuenta Ajena Contingencia",
    241: "e-Factura Venta por Cuenta Ajena Contingencia",
    242: "Nota de CrÃ©dito de e-Factura Venta por Cuenta Ajena Contingencia",
    243: "Nota de DÃ©bito de e-Factura Venta por Cuenta Ajena Contingencia",
    251: "e-Boleta Contingencia",
    252: "Nota de CrÃ©dito e-Boleta Contingencia",
    253: "Nota de DÃ©bito e-Boleta Contingencia",
...
def parse_cfe_xml(xml_bytes: bytes, default_adenda: str = "") -> CFEData:
    ...
    forma_pago = "Contado" if fma_pago == "1" else ("CrÃ©dito" if fma_pago == "2" else (fma_pago or ""))
    ...
    # Sucursal DGI (CdgDGISucur) - se usa para desempatar marcas cuando RUC/RS son iguales.
    emisor_sucursal: Optional[int] = None
    try:
        suc = _find_text(comprobante, "cfe:Encabezado/cfe:Emisor/cfe:CdgDGISucur", ns)
        suc = (suc or "").strip()
        if suc:
            emisor_sucursal = int(re.findall(r"\d+", suc)[0])
    except Exception:
    ...
    # CÃ³digo de seguridad (corto) - best-effort deterministic
    ...
    return CFEData(
        ...
        emisor_sucursal=emisor_sucursal,
...
# ------------------------------------------------------------
# AdaptaciÃ³n para "Iluminaras Suite": reusa la lÃ³gica legacy del
# proyecto CFE autopoll (ticket + ticket de cambio) como motor.
# Este mÃ³dulo NO registra rutas Flask.
# ------------------------------------------------------------
from types import SimpleNamespace

app = SimpleNamespace(config={}, root_path=os.getcwd())
app_log = logging.getLogger("cfe_legacy_engine")

# caches (antes estaban dentro de create_app)
...
def configure(config: dict[str, Any], root_path: str | None = None, logger: logging.Logger | None = None) -> None:
    """Configura el motor con un dict estilo Flask app.config.

    Nota: si el caller no define rutas de logos, se asignan defaults (y se permite override por ENV).
    """
    app.config = config
    if root_path:
    ...
    # --- Defaults de logos (compatibles con los otros programas) ---
    # LOGO_PATH: fallback histórico (Iluminaras)
    if not str(app.config.get("LOGO_PATH", "") or "").strip():
        app.config["LOGO_PATH"] = os.environ.get(
            "LOGO_PATH",
            os.path.join(app.root_path, "static", "logo.png"),
        )

    # LOGO_LUMINARAS_PATH: por marca Iluminaras (si no está, usar LOGO_PATH)
    if not str(app.config.get("LOGO_LUMINARAS_PATH", "") or "").strip():
        app.config["LOGO_LUMINARAS_PATH"] = os.environ.get(
            "LOGO_LUMINARAS_PATH",
            str(app.config.get("LOGO_PATH", "") or "").strip() or os.path.join(app.root_path, "static", "logo.png"),
        )

    # LOGO_ESTILO_HOME_PATH: por marca Estilo Home / REINE
    if not str(app.config.get("LOGO_ESTILO_HOME_PATH", "") or "").strip():
...
def _estimate_receipt_height_pt(cfe: CFEData, w_pt: float) -> float:
        ...
        # Buyer details: count only non-empty lines (Nombre, DirecciÃ³n, Ciudad)
        details_lines = 0
        ...
        for v in (cfe.receptor_nombre, cfe.receptor_direccion, ciudad):
            if (v or "").strip():
                details_lines += 1
        fixed += details_lines * mm(4.8) + mm(8)
...
def _estimate_change_ticket_height_pt(cfe: CFEData, w_pt: float) -> float:
        ...
        policy_url = _policy_url_for_emisor(cfe.emisor_razon_social, cfe.emisor_ruc, cfe.emisor_sucursal)
        if footer_txt:
...
def generate_receipt_pdf(cfe: CFEData, override_path: Optional[str] = None) -> str:
        ...
        # --- Header (logo + issuer) ---
        try:
            logo_path = _logo_path_for_emisor(cfe.emisor_razon_social, cfe.emisor_ruc, cfe.emisor_sucursal)
        ...
        # Documento fiscal: si el nombre del tipo es mÃ¡s largo que el ancho del ticket,
        # hacer saltos de lÃ­nea para que no se corte.
        doc_label = _sanitize_doc_type_label(cfe.tipo_texto)
        ...
        def draw_lv(label: str, value: str) -> None:
            ...
            c.drawString(x0 + mm(20), y, v[:120])

        draw_lv("Nombre", cfe.receptor_nombre)
        draw_lv("DirecciÃ³n", cfe.receptor_direccion)
        ciudad = " ".join([p for p in [cfe.receptor_ciudad, cfe.receptor_depto] if p]).strip()
        ...
        if cfe.codigo_seguridad_corto:
            c.drawCentredString(x0 + inner_w / 2, y, f"CÃ³digo de Seguridad: {cfe.codigo_seguridad_corto}")
            y -= mm(5)
        ...
        c.drawCentredString(x0 + inner_w / 2, y, "IVA al dÃ­a")
        y -= mm(5.0)
...
def generate_change_ticket_pdf(cfe: CFEData, override_path: Optional[str] = None) -> str:
        ...
        if _brand_from_emisor(cfe.emisor_razon_social, cfe.emisor_ruc, cfe.emisor_sucursal) == "ESTILO_HOME":
            # Estilo Home (REINE): no imprimir el link de polÃ­tica de cambios de Iluminaras
            policy_url = ""
        if policy_url:
            c.setFont("Helvetica", 8.6)
            y -= mm(4.6)
        ...
        y -= mm(5.2); c.drawString(x0, y, f"Cambio vÃ¡lido hasta: {valido}")
        ...
        if policy_url:
            y -= mm(5.0)
            c.setFont("Helvetica-Bold", 8.8)
            c.drawString(x0, y, "PolÃ­tica completa en:")
            y -= mm(4.6)
            c.setFont("Helvetica", 8.8)
...
def _odoo_download_attachment(att_id: int) -> tuple[str, bytes, str]:
    ...
    try:
        ...
        # Error tÃ­pico transitorio de xmlrpc (por ejemplo: "Idle", "Request-sent")
        msg = str(ex)
...
def _odoo_recent_cfe_xml_attachments(limit: int) -> list[dict[str, Any]]:
    ...
    # --- Strategy 0: account.move (Accounting > Invoicing) ---
    try:
        move_fields = _model_fields("account.move")
    except Exception:
        move_fields = set()
    ...
    if xml_field or doc_field:
        ...
        # IMPORTANT: l10n_uy_edi_xml_attachment_id can be store=False (non-searchable) in some Odoo builds,
        # so we MUST NOT use it in domain filters; only read it if present.
        if doc_field:
...
def _default_adenda_for_emisor(razon_social: str, ruc: str | None = None, sucursal: int | None = None) -> str:
    # Use the same brand routing as logo selection (prefer issuer RUC).
    brand = _brand_from_emisor(razon_social, ruc, sucursal)
    if brand == "ESTILO_HOME":
        return str(app.config.get("DEFAULT_ADENDA_ESTILO_HOME", "") or "").strip()
    if brand == "MAYORISTAS_URUGUAY":
        return str(app.config.get("DEFAULT_ADENDA_MAYORISTAS_URUGUAY", "") or "").strip()
    return str(app.config.get("DEFAULT_ADENDA_LUMINARAS", "") or "").strip()




def _policy_url_for_emisor(razon_social: str, ruc: str | None = None, sucursal: int | None = None) -> str:
    """Policy URL routing por empresa.
    ...
    Fallback:
      - CHANGE_TICKET_POLICY_URL (global)
    """
    brand = _brand_from_emisor(razon_social, ruc, sucursal)
    if brand == "ESTILO_HOME":
        return str(app.config.get("CHANGE_TICKET_POLICY_URL_ESTILO_HOME", "") or "").strip()
    if brand == "MAYORISTAS_URUGUAY":
        return str(app.config.get("CHANGE_TICKET_POLICY_URL_MAYORISTAS_URUGUAY", "") or "").strip() or str(app.config.get("CHANGE_TICKET_POLICY_URL", "") or "").strip()
    # Default Iluminaras
    return str(app.config.get("CHANGE_TICKET_POLICY_URL_LUMINARAS", "") or "").strip() or str(app.config.get("CHANGE_TICKET_POLICY_URL", "") or "").strip()


def _logo_path_for_emisor(razon_social: str, ruc: str | None = None, sucursal: int | None = None) -> str:
    # Use the same brand routing as adenda/policy logic (prefer issuer RUC).
    brand = _brand_from_emisor(razon_social, ruc, sucursal)
    if brand == "ESTILO_HOME":
        p = str(app.config.get("LOGO_ESTILO_HOME_PATH", "") or "").strip()
    elif brand == "MAYORISTAS_URUGUAY":
        p = str(app.config.get("LOGO_MAYORISTAS_URUGUAY_PATH", "") or "").strip()
    else:
        p = str(app.config.get("LOGO_LUMINARAS_PATH", "") or "").strip()
    # Backwards compatible fallback
    p = p or str(app.config.get("LOGO_PATH", "") or "").strip()
    return _resolve_path(app.root_path, p) if p else ""

def _odoo_get_cfe_xml_from_move(move_id: int) -> tuple[int, bytes]:
    """Fallback: busca un XML CFE en ir.attachment ligado a account.move."""
    if move_id <= 0:
        raise RuntimeError("move_id_invalid")
    db, uid, api_key, common, models = _odoo_client()

    # Buscar attachments XML en el move (prioriza nombres con CFE)
    domain = [
        ("res_model", "=", "account.move"),
        ("res_id", "=", int(move_id)),
        ("type", "=", "binary"),
        "|", ("mimetype", "=", "application/xml"), ("name", "ilike", ".xml"),
    ]
    ...
    recs = models.execute_kw(
        ...
        "ir.attachment", "search_read",
        [domain],
        {"fields": fields, "limit": 20, "order": "create_date desc"},
    ) or []

    if not recs:
        raise RuntimeError("move_no_xml_attachments")

    # preferir CFE si aparece en el nombre
    recs_sorted = sorted(recs, key=lambda r: (0 if "cfe" in (r.get("name","").lower()) else 1, r.get("create_date") or ""), reverse=False)
    att_id = int(recs_sorted[0]["id"])
    _, data, _ = _odoo_download_attachment(att_id)
    if not data:
        raise RuntimeError("move_attachment_empty")
    return att_id, data
