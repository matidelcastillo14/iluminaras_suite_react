...
# ---------------------------------------------------------------------------
# JSON API endpoints for Admin
#
# The React frontend relies on JSON endpoints to manage users,
# roles, settings and modules.  The legacy admin blueprint only
# exposed HTML forms, so we provide thin wrappers here that work
# with the existing models and helper functions.  All API routes
# are protected by authentication and require the admin role.
...
def api_users_list():
    ...
    for u in users:
        out.append({
            ...
            "phone": u.phone,
            "attendance_ref_code": u.attendance_ref_code,
            "home_office_clock_enabled": bool(u.home_office_clock_enabled),
            "role": u.role,
...
def api_users_create():
    """Create a new user via JSON payload."""
    ...
    attendance_ref_code = (payload.get("attendance_ref_code") or "").strip()
    home_office_clock_enabled = bool(payload.get("home_office_clock_enabled"))
    ...
    u = User(
        ...
        home_office_clock_enabled=home_office_clock_enabled,
        role=role,
        is_active=True,
    ...
    body = f"Usuario creado: {username}\nContraseña temporal: {tmp}\n\nAl iniciar sesión se te solicitará cambiarla."
    sent = _send_email(email, "Acceso Iluminaras Suite", body)
    return jsonify({"id": u.id, "temp_password": tmp, "email_sent": bool(sent)}), 201
...
def api_users_update(user_id: int):
    """Update an existing user.  Accepts partial JSON."""
    u = User.query.get_or_404(user_id)
    payload = request.get_json(silent=True) or {}
    for key, val in payload.items():
        if key == "role":
            if val not in set(_all_roles()):
                continue
            setattr(u, key, val)
        elif key in {"username", "email", "first_name", "last_name", "phone", "attendance_ref_code"}:
            setattr(u, key, val)
        elif key == "home_office_clock_enabled":
            setattr(u, key, bool(val))
        elif key == "is_active":
            setattr(u, key, bool(val))
    db.session.commit()
    return jsonify({"ok": True})
...
@login_required
@admin_required
def api_users_toggle(user_id: int):
    """Toggle the active status of a user."""
    u = User.query.get_or_404(user_id)
    u.is_active = not bool(u.is_active)
    db.session.commit()
    return jsonify({"id": u.id, "is_active": bool(u.is_active)})


@bp.post("/api/users/<int:user_id>/reset-temp")
@login_required
@admin_required
def api_users_reset_temp(user_id: int):
    ...
    body = f"Tu contraseña fue restablecida.\nUsuario: {u.username}\nContraseña temporal: {tmp}\n\nAl iniciar sesión se te solicitará cambiarla."
    sent = _send_email(u.email, "Restablecer contraseña (Iluminaras Suite)", body)
    return jsonify({"id": u.id, "temp_password": tmp, "email_sent": bool(sent)})
...
def api_settings_list():
    """Return current settings values for known keys."""
    values = {k: Setting.get(k, str(current_app.config.get(k, ""))) for k in SETTINGS_KEYS}
...
def api_settings_update():
    """Update multiple settings from JSON payload."""
    payload = request.get_json(silent=True) or {}
    ...
    for k, v in payload.items():
        if k not in SETTINGS_KEYS:
            continue
        Setting.set(k, str(v) if v is not None else "")
    try:
        db.session.commit()
    except Exception:
        db.session.rollback()
...
def api_modules_list():
    """List all modules and their enabled flags."""
    mods = []
    for m in all_modules():
        mods.append({
            "key": m.key,
            "name": m.label,
            "description": m.label,
            "internal_enabled": bool(is_module_internal_enabled(m.key)),
            "public_enabled": (bool(is_module_public_enabled(m.key)) if m.has_public else None),
        })
    return jsonify(mods)


@bp.post("/api/modules/<string:key>/toggle")
@login_required
@admin_required
def api_module_toggle(key: str):
    """Toggle a module's internal/public flags."""
    ...
    if internal is not None:
        Setting.set(internal_setting_key(key), "1" if internal else "0")
    if mdef.has_public and public is not None:
        Setting.set(public_setting_key(key), "1" if public else "0")
    try:
        db.session.commit()
    except Exception:
        db.session.rollback()
    ...
    return jsonify({
        "key": key,
        "internal_enabled": bool(is_module_internal_enabled(key)),
        "public_enabled": (bool(is_module_public_enabled(key)) if mdef.has_public else None),
    })
