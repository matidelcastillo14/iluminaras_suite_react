...
# ---------------------------------------------------------------------------
# JSON API endpoints for Tracking (Rastreo)
#
# These endpoints expose shipment details and allow state changes
# without requiring a full page refresh.  They are used by the new
# React frontend.  Access is restricted via the existing view
# permissions: ``rastreo_deposito`` or ``rastreo_cadeteria`` for
# general tracking, and ``rastreo_ventas`` for admin tracking.


@bp.get("/api/shipments/<string:code>")
@login_required
def api_shipment_detail(code: str):
    """Return shipment information and its events.

    The code may correspond to a tracking code, order name or id_web.
    Returns 404 if no shipment is found.  Only users with access to
    any tracking view can call this endpoint.
    """
    # Ensure the user has at least one tracking view permission
    if not (can_view("rastreo_deposito") or can_view("rastreo_cadeteria") or can_view("rastreo_ventas")):
        abort(403)
    sh = _find_shipment_by_any_code(code)
    if not sh:
    ...
    shipment = {
        ...
        "id_web": sh.id_web,
        "status": sh.status,
        "courier_user_id": sh.courier_user_id,
        "warehouse_user_id": sh.warehouse_user_id,
    }
    # Get events ordered descending
    events = []
    ev_rows = TrackingEvent.query.filter_by(shipment_id=sh.id).order_by(TrackingEvent.created_at.asc(), TrackingEvent.id.asc()).all()
    for ev in ev_rows:
        try:
    ...
    return jsonify({"shipment": shipment, "events": events})


@bp.post("/api/shipments/<string:code>/event")
@login_required
def api_shipment_add_event(code: str):
    """Record a new event on a shipment.

    Accepts JSON with keys ``event_type`` and optional ``note``.  The
    caller must have a tracking view permission appropriate for
    emitting the event (see services/tracking_service).  Returns the
    new event ID on success.
    """
    if not (can_view("rastreo_deposito") or can_view("rastreo_cadeteria") or can_view("rastreo_ventas")):
        abort(403)
    sh = _find_shipment_by_any_code(code)
    ...
    try:
        ev = add_event(sh, event_type=event_type, note=note, created_by_user_id=current_user.id)
    except ValueError as ex:
        # The tracking service raises ValueError for business rule violations
        return jsonify({"error": str(ex)}), 400
    return jsonify({"ok": True, "event_id": ev.id})


@bp.get("/api/admin/shipments/search")
@login_required
@view_required("rastreo_ventas")
def api_admin_shipments_search():
    """Search shipments for admin tracking.

    Accepts query parameter ``q`` and returns a list of matching
    shipments.  Matches by tracking code, order name or id_web.
    """
    q = (request.args.get("q") or "").strip()
    if not q:
        return jsonify([])
    like = f"%{q}%"
    rows = (
        TrackingShipment.query.filter(
            (TrackingShipment.tracking_code.ilike(like))
            | (TrackingShipment.order_name.ilike(like))
            | (TrackingShipment.id_web.ilike(like))
        )
        .order_by(TrackingShipment.created_at.desc())
        .limit(50)
        .all()
    )
    out = []
    for sh in rows:
        out.append({
            "code": sh.tracking_code or sh.order_name or sh.id_web,
            "tracking_code": sh.tracking_code,
            "order_name": sh.order_name,
            "id_web": sh.id_web,
            "status": sh.status,
...
@bp.get("/api/admin/shipments/<string:code>")
@login_required
@view_required("rastreo_ventas")
def api_admin_shipment_get(code: str):
    """Return detailed shipment info for admin tracking."""
    sh = _find_shipment_by_any_code(code)
    if not sh:
        return jsonify({"error": "not_found"}), 404
    # Build detail using same helper as user view
    shipment_resp = api_shipment_detail(code)
    # The above returns a Flask response; extract JSON payload
    return shipment_resp


@bp.post("/api/admin/shipments/<string:code>/override")
@login_required
@view_required("rastreo_ventas")
def api_admin_shipment_override(code: str):
    """Override a shipment's status via sales/admin.

    Accepts JSON with key ``status`` and optional ``note``.  Uses
    ``ventas_override_status`` from the tracking_service to bypass
    the state machine.  Returns the updated shipment details.
    """
    payload = request.get_json(silent=True) or {}
    ...
    try:
        ventas_override_status(code, new_status=new_status, created_by_user_id=current_user.id, note=note)
    except ValueError as ex:
        return jsonify({"error": str(ex)}), 400
    # Return updated record
    return api_admin_shipment_get(code)


@bp.post("/api/admin/shipments/<string:code>/reset")
@login_required
@view_required("rastreo_ventas")
def api_admin_shipment_reset(code: str):
    """Reset a shipment to its initial state via sales/admin."""
    try:
        ventas_reset_to_zero(code)
    except ValueError as ex:
        return jsonify({"error": str(ex)}), 400
    return api_admin_shipment_get(code)
