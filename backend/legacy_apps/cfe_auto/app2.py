...
def create_app() -> Flask:
    ...
    def _odoo_recent_cfe_xml_attachments(limit: int) -> list[dict[str, Any]]:
        ...
        if xml_field or doc_field:
            ...
            if xml_field and doc_field:
                domain += ["|", (xml_field, "!=", False), (doc_field, "!=", False)]
            elif xml_field:
                domain.append((xml_field, "!=", False))
            elif doc_field:
    ...
    def _logo_path_for_emisor(razon_social: str) -> str:
        """Return logo path using the same brand logic as app.py.

        NOTE: This is intentionally based ONLY on razon_social (issuer name),
        to match the original app.py behavior.
        """
        rs = (razon_social or "").strip().upper()
        rs = re.sub(r"[^A-Z0-9]+", " ", rs)
        rs = re.sub(r"\s+", " ", rs).strip()

        brand = "ESTILO_HOME" if re.search(r"\bREINE\b", rs) else "LUMINARAS"
        if brand == "ESTILO_HOME":
